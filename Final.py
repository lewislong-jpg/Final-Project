...
# ── Run app ─────────────────────────────────────────────
# Only run analysis when the button is pressed.
# Use session_state to persist results across reruns (slider moves).
if run_btn:
...
if st.session_state.get("run_analysis", False):
    # Use the locked-in params from when Analyze was pressed
    ...
    fig.add_trace(go.Scatter(x=df["Date"], y=df["MA_Short"], name="MA Short", line=dict(dash="dot")))
    fig.add_trace(go.Scatter(x=df["Date"], y=df["MA_Long"], name="MA Long", line=dict(dash="dot")))
    ...
    # ── Signal summary (now properly indented INSIDE the if block) ──
    ...
    st.info("👈 Adjust your settings in the sidebar and click **Analyze** to begin.")
